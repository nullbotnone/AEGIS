...
from src.paper.support.constraints import create_ablation_constraints, ConstraintProfile
from src.paper.support.logger import ActionLogger, ActionType
from src.paper.support.attestation import AttestationEngine, ThreatLevel
from src.paper.attacks.ablation_attack1_volume import AblationVolumeAttack
from src.paper.attacks.ablation_attack2_sensitive import AblationSensitiveAttack
from src.paper.attacks.ablation_attack3_covert import AblationCovertChannelAttack
from src.paper.attacks.ablation_attack4_injection import AblationToolInjectionAttack
...
def _serialize_results(all_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    serialized: Dict[str, Any] = {}
    for config_name, data in all_results.items():
        serialized[config_name] = {
            "attacks_detected": data["attacks_detected"],
            "total_attacks": data["total_attacks"],
            "detection_rate": data["detection_rate"],
            "results": {attack_name: asdict(result) for attack_name, result in data["results"].items()},
        }
    return serialized
...
def run_experiment(output: Optional[str] = None):
    ...
    payload = {
        ...
        "experiment": "simulated_ablation",
        "results": _serialize_results(all_results),
    }
    output_path = Path(output) if output else (
        Path("results") / f"simulated_ablation_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the AEGIS synthetic ablation study")
