...
\projectname~is fully implemented as a modular system. The implementation consists of approximately 4,500 lines of code. The \textit{ebpf} probe in C provides kernel-side syscall hooks. The Python collector reads from the ring buffer and produces evidence bundles. The cross-node coordinator implements cluster-wide correlation for covert channel detection. The baseline comparisons module provides comparison against DLP, audit, analytics, and sandbox defenses. The Slurm integration module provides containment through the REST API. The implementation is available as open source.

% The TPM attestation module supports hardware-rooted signing with TPM 2.0. 
