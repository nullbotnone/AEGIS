...
This paper identifies and formalizes a threat that has not been studied in prior work: **the hijacked authorized agent in HPC**. Unlike rogue agents (which existing authentication blocks) or adversarial agents (which existing authorization constrains), a hijacked agent operates with the full credentials and privileges of a legitimate user. It does not need to escalate privileges; it already has them. It does not need to bypass access controls; it is already authorized. It appears legitimate to every monitoring system that checks identity, as its identity is legitimate. Yet it exfiltrates data through the one channel no HPC system inspects: the encrypted LLM API connection the agent requires to function.
